...
class KoopmanEngine:
    ...
    Attributes:
        ...
        model (KoopmanModel): Koopman model for prediction
        data_loader (OmicsDataloader): Data loader for OMICS data
    ...
    def __init__(self, config: Union[Dict[str, Any], str, ConfigManager] = None):
        """
        Initialize the KoopmanEngine model.
        ...
        logger.info("KOOPOMICS engine initialized with configuration.")
    ...
    def load_data(self, data: Union[pd.DataFrame, torch.Tensor, Tuple[torch.Tensor, torch.Tensor]],
                 ...
                 condition_id: Optional[str] = None) -> None:
        """
        ...
        if isinstance(data, tuple) and len(data) == 2 and all(isinstance(t, torch.Tensor) for t in data):
            logger.info("Loading pre-split train and validation data...")
    ...
    def train(self, data: Optional[Union[pd.DataFrame, torch.Tensor]] = None,
        ...
        best_metric = self.trainer.train()
        logger.info(f"Training completed with best metric: {best_metric:.6f}")
